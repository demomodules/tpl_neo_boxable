...
class tplconfig {
    ...
    function __construct() {
        ...
        if (defined('MODULE_TPLCONFIG_NEO_VERSION')) $this->title .= ' <small>(TPL-Version: '.constant('MODULE_TPLCONFIG_NEO_VERSION').')</small>';
        if (defined('MODULE_TPLCONFIG_STATUS') && MODULE_TPLCONFIG_STATUS == 'true') $this->description .= '<a class="button btnbox" style="background-color:#6e9c25;border:1px solid #6e9c25;text-align:center;" onclick="this.blur();" href="'.xtc_href_link(FILENAME_TPLCONFIG, 'gID=601', 'NONSSL').'">TPLManager &raquo;';
        if (defined('MODULE_TPLCONFIG_STATUS')) { if (defined('MODULE_TPLCONFIG_NEO_VERSION') && MODULE_TPLCONFIG_NEO_VERSION < $this->version || !defined('MODULE_TPLCONFIG_NEO_VERSION')) $this->description .= '<a class="button btnbox" style="background-color:#d34040;border:1px solid #d34040;text-align:center;" onclick="this.blur();" href="'.xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module='.$this->code.'&action=update').'">Update</a>'; }
    }
    ...
    function install() {
		...
		xtc_db_query("INSERT INTO ".TABLE_CONFIGURATION." (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_TPLCONFIG_NEO_VERSION', '".$this->version."', '6', '2', now())");
    ...
    function update() {
		...
		if (!defined('MODULE_TPLCONFIG_NEO_VERSION')) xtc_db_query("INSERT INTO ".TABLE_CONFIGURATION." (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_TPLCONFIG_NEO_VERSION', '".$this->version."', '6', '2', now())");
		if (defined('MODULE_TPLCONFIG_NEO_VERSION') && MODULE_TPLCONFIG_NEO_VERSION < $this->version) xtc_db_query("UPDATE ".TABLE_CONFIGURATION." SET configuration_value = '".$this->version."' WHERE configuration_key = 'MODULE_TPLCONFIG_NEO_VERSION'");
        // new configuration keys for group 601
        // new configuration keys for group 602
        // new configuration keys for group 603
        // new configuration keys for group 604
        // new configuration keys for group 605
		if (!defined('TPL_CONFIG_SHORTDESCRIPTION')) xtc_db_query("INSERT INTO ".TABLE_CONFIGURATION." (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('TPL_CONFIG_SHORTDESCRIPTION', 'false',  '605', '2', 'xtc_cfg_select_option(array(\'true\', \'false\'), ', now())"); 
        // new configuration keys for group 606
        // new configuration keys for group 607
    }
