...
/* ------------------------------------------------------------------------------------------------------------------
   ...
   CSS Produkt- & Attributlagerampel v1.4 (2023-08-26)
   ...
   -------------------
     ...
     product_attributes:{traffic_light stock=$item_data.STOCK modul='attributes'}
...
function smarty_function_traffic_light($params, &$smarty) {
  ...
  if (defined('MODULE_TRAFFIC_LIGHTS_STATUS') && MODULE_TRAFFIC_LIGHTS_STATUS == 'true') {
    ...
    if (constant('MODULE_TRAFFIC_LIGHTS_ATTRIBUTES') == 'true' && $modul == 'attributes') {
      ...
      $show_stock = defined('MODULE_TRAFFIC_LIGHTS_ATTRIBUTES_SHOW_STOCK') && MODULE_TRAFFIC_LIGHTS_ATTRIBUTES_SHOW_STOCK == 'true' ? (($stock < 1) ? '' : ' | '.$stock) : '';
      ...
      if ($stock < MODULE_TRAFFIC_LIGHTS_STOCK_RED_YELL) {
          $stock_info = '<span class="traff-light"><span class="tl red-tl"></span>'.(($flow_in == true) ? '<span class="nr-tooltip red-tl" aria-label="'.MODULE_TRAFFIC_LIGHTS_STOCK.'">'.MODULE_TRAFFIC_LIGHTS_QTY_RED.$show_stock.'</span>' : '').'</span>';
          $html .= $stock_info;
      } elseif ($stock >= MODULE_TRAFFIC_LIGHTS_STOCK_RED_YELL && $stock < MODULE_TRAFFIC_LIGHTS_STOCK_GREEN) {
          $stock_info = '<span class="traff-light"><span class="tl yell-tl"></span>'.(($flow_in == true) ? '<span class="nr-tooltip yell-tl yell-txt" aria-label="'.MODULE_TRAFFIC_LIGHTS_STOCK.'">'.MODULE_TRAFFIC_LIGHTS_QTY_YELL.$show_stock.'</span>' : '').'</span>';
          $html .= $stock_info;
      } elseif ($stock >= MODULE_TRAFFIC_LIGHTS_STOCK_GREEN) {
          $stock_info = '<span class="traff-light"><span class="tl green-tl"></span>'.(($flow_in == true) ? '<span class="nr-tooltip green-tl" aria-label="'.MODULE_TRAFFIC_LIGHTS_STOCK.'">'.MODULE_TRAFFIC_LIGHTS_QTY_GREEN.$show_stock.'</span>' : '').'</span>';
          $html .= $stock_info;
      }
