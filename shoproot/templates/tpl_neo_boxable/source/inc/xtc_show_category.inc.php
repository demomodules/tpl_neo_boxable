    ...
    if (!isset($category_data_array[$parent_id])) {
      ...
      if (xtc_db_num_rows($categories_query, true) > 0) {
        while ($categories = xtc_db_fetch_array($categories_query, true)) {
	      if (defined('MODULE_CATEGORIES_FLAG_STATUS') && MODULE_CATEGORIES_FLAG_STATUS == 'true') {
	        $flag = (($categories['flags'] > 0) ? '<span class="categoryFlag'.$categories['flags'].'">'.constant('CATEGORIES_FLAG_'.$categories['flags']).'</span>' : '');
	      } else {
	        $flag = '';
	      }
  ...
  function xtc_show_category($parent_id = 0, $path = '', $category_tree_array = array()) {
    ...
    foreach ($category_tree_array[$parent_id] as $categories) {
      if (mod_count_products_in_category($categories['id']) > 0) {
	    ...
	    if (isset($categories['flags'])) {
	      $categories_string .= $categories['flags'];  
	    }
